...
Redesignet tager udgangspunkt i \textbf{problem \#3 fra den heuristiske evaluering}: det
overfyldte produktkort med mange farver, ikoner og knapper, der skjuler den primære handling
``Læg i kurv'' i visuel støj (H8, H4). Forslaget er et \emph{forenklet produktkort med
...
\begin{tikzpicture}[font=\scriptsize, x=1cm, y=1cm]
  ...
  \node[gray!60] at (1.8,5.0) {\itshape billede};
...
Ifølge Hick's lov stiger valgreaktionstiden logaritmisk med antallet af valgmuligheder
...
($t_P \propto \log_2(2D/W)$; Hornbæk et al., 2025, kap.\ 7). Det nuværende interface kræver
...
musemarkørens aktuelle position --- et stort, let tilgængeligt mål. KLM-analysen
kvantificerer besparelsen (Bilag~B og~C).

\textbf{Affordance og visuel hierarki (brugergrænseflader):}
...
(Gibson, 1977; jf.\ Hornbæk et al., 2025, kap.\ 13): dens fremtræden kommunikerer
...
(nuværende) og Bilag~C (redesign). Opsummering af komponent-tider:
...
For at vurdere om KLM-resultaterne er gyldige, foreslår jeg et \textbf{kontrolleret
laboratorieforsøg} med \textbf{12--15 erfarne webshoppere} (mindst ét online
dagligvareindkøb pr.\ måned, screenet via spørgeskema). KLM antager ekspertydeevne;
deltagerne gennemfører én øvetrial per interface for at tilnærme sig dette.

\textbf{Design:} Within-subjects med counterbalanced rækkefølge (AB/BA), så halvdelen
starter med nuværende og halvdelen med redesignet. Ordenseffekter kontrolleres herved.

\textbf{Opgave og måling:} Præcis den KLM-analyserede opgave: ``Søg efter `mælk' og læg
én mælk i kurven'' (start: forsiden). Tid måles fra opgaveinstruktion til klik på
kurv-knappen. Fejlbehæftede trials ekskluderes, da KLM antager fejlfri adfærd.

\textbf{Analyse og validitetskriterium:} Observerede tider sammenlignes med
KLM-for\-ud\-si\-gel\-ser\-ne (11{,}60\,s og 9{,}30\,s) via \emph{one-sample t-test}. KLM betragtes
som valid, hvis observerede gennemsnit falder inden for $\pm5$\,s af prediktion (jf.\
Hornbæk et al., 2025, s.\ 736). Forskel mellem interfaces testes med \emph{paired t-test}.
En central trussel mod validiteten er KLMs antagelse om eksperter og fejlfri adfærd: de
fleste brugere er ikke eksperter, og observerede tider vil sandsynligvis overstige
predik\-tionerne systematisk. Den \emph{relative} besparelse (2{,}30\,s) forventes dog
robust, da antagelserne gælder begge interfaces.
...
\noindent\textbf{Opgave:} ``Søg efter `mælk' og læg én mælk i kurven'' (start: Nemlig.coms forside).\\
